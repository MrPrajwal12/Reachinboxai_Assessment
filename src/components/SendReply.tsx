...
import { sendMailMessages } from '../actions/actions';  // Corrected function name

interface SendReplyProps {
    handleCancel: MouseEventHandler<HTMLDivElement>;
    currColor: boolean;
    singleMail: any;  // Ideally, replace `any` with a specific type
}
...
const SendReply: React.FC<SendReplyProps> = ({ currColor, handleCancel, singleMail }) => {
    ...
    const handleSubmit = () => {
        if (token) {
            sendMailMessages(singleMail.threadId, formData)
                .then((res) => {
                    alert("Reply has been sent");
                })
                .catch((err: Error) => console.error("Error sending reply:", err));
        } else {
        ...
        <div className='flex flex-col w-full h-full'>
            ...
            <div className='h-[54px] w-full'>
                ...
                <div className='flex text-[12px] gap-1'>
                    <div className='w-[100px] h-8 bg-[#4B63DD] flex items-center ml-4 mb-3 rounded gap-2 justify-center cursor-pointer'>
                        <button className='text-white' onClick={handleSubmit}>Send</button>
                        <ChevronDown color={"white"} />
                    </div>
                    <div className='w-[100px] h-8 flex items-center mb-3 rounded gap-2 justify-center cursor-pointer'>
                        <Zap className='h-4' />
                        <button>Variables</button>
                    </div>
                    <div className='w-[120px] h-8 flex items-center mb-3 rounded gap-2 justify-center cursor-pointer'>
                        <Eye className='h-4' />
