...
import { fetchMailList, fetchMailMessages, removeMailResponse } from '../actions/actions';
...
const Desktop = () => {
    ...
    const fetchData =()=>{
        fetchMailList(token).then(res => {
        console.log(res)
           setData(res);
            if (res?.length > 0) {
               setSingleMail(res[0]);
                const id: number = res[0]?.threadId;
                if (id !== undefined)  return fetchMailMessages(id);
    ...
    const handleChangeEmail = (id: number) => {
        fetchMailMessages(id).then(messages =>{
    ...
    const deleteEmail =()=>{
        ...
        removeMailResponse(id).then((res)=>{
                        ...
                        <div className=' text-left'>
                            {
                                data?.length >0 && data.map((item:any) =>{
                                    return <div key={item.id}>
                                        <EmailCard currColor = {currColor} {...item} handleChangeEmail={handleChangeEmail}/>
                                        <hr />
                                    </div>
                                })
                            }
